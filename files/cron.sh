...
#TROQUE
  ...
  if [ "${PANEL}" = "Jexactyl" ] || [ "${PANEL}" = "Jexactyl Brasil" ]; then
     if [ "$HORA" == "$HORA_CERTA" ]; then
       php /home/container/pterodactyl/artisan p:schedule:renewal >> /dev/null 2>&1
     fi
  fi
  ...
  php /home/container/pterodactyl/artisan schedule:run >> /dev/null 2>&1 && /bin/bash /crontab_test.sh
